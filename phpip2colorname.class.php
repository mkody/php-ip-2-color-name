...
class phpIp2ColorName {
	 ...
	 * @param ip $method
	 */
	function __construct($ip,$method=1){
		...
		if( $this->method == 1 ){
				// first tree octets of IP to RGB method
				$this->rgbArr = array(
					$this->ipArr[0],
					$this->ipArr[1],
					$this->ipArr[2]
				...
				// IP to CMYK to RGB conversion (simple method)
				$this->rgbArr = $this->cmyk_to_rgb(
					$this->ipArr[0],
					$this->ipArr[1],
					$this->ipArr[2],
					$this->ipArr[3]
				...
				// IP to CMYK to RGB conversion (Voisen.org method)
				$this->rgbArr = $this->cmyk_to_rgb2(
					$this->ipArr[0],
					$this->ipArr[1],
					$this->ipArr[2],
					$this->ipArr[3]
				...
				// IP to CMYK to RGB conversion (Wikipedia method)
				$this->rgbArr = $this->cmyk_to_rgb3(
					$this->ipArr[0],
					$this->ipArr[1],
					$this->ipArr[2],
					$this->ipArr[3]
	...
	/**
	 * rgb representation of IP address form of array of integer values
	 *
	 * @var string
	 */
	...
	/**
	 * index form colors array for color most similar to given IP address
	 *
	 * @var string
	 */
	...
	private function determineColor(){
		...
		for($i=0;$i<$count;$i++){
			$this->colors[$i][6] =
				abs( $this->colors[$i][3] - $this->rgbArr[0] )
				+ abs( $this->colors[$i][4] - $this->rgbArr[1] )
				+ abs( $this->colors[$i][5] - $this->rgbArr[2] );
	...
	/**
	 * returns HTML color name
	 *
	 * @return string
	 */
	public function getColorRealName(){
		if(!is_null($this->colorIndex) && isset($this->colors[$this->colorIndex]) ){
			return $this->colors[$this->colorIndex][0];
		}else{
			return '';
		}
	}
	 ...
	 * @return string
	 */
	public function getColorReadableName(){
		if(!is_null($this->colorIndex) && isset($this->colors[$this->colorIndex]) ){
	...
	/**
	 * returns inverted HTML color hex value
	 *
	 * @param unknown_type $prefix
	 * @return string
	 */
	public function getInvertedColorHexValue($prefix='#'){
		if(!is_null($this->colorIndex) && isset($this->colors[$this->colorIndex]) ){
			$r = dechex(255-$this->colors[$this->colorIndex][3]);
			$g = dechex(255-$this->colors[$this->colorIndex][4]);
			$b = dechex(255-$this->colors[$this->colorIndex][5]);
			if(strlen($r)==1) $r = '0'.$r;
			if(strlen($g)==1) $r = '0'.$g;
			if(strlen($b)==1) $r = '0'.$b;
			return $prefix.strtoupper($r.$g.$b);
		}else{
			return '';
		}
	}
	 ...
	 * @return array
	 */
	private function cmyk_to_rgb($c, $m, $y, $k)
     {
	 ...
	 * @return array
	 */
	private function cmyk_to_rgb2($c, $m, $y, $k)
     {
    	$c = (255 * $c) / 100;
    	$m = (255 * $m) / 100;
    	$y = (255 * $y) / 100;
	 ...
	 * @return array
	 */
	private function cmyk_to_rgb3($c, $m, $y, $k)
	{	
		$c	= $c/100 ;
		$m	= $m/100 ;
		$y	= $y/100 ;
